...
import { ChatIntakeForm } from '@/components/nexrena-chat/ChatIntakeForm';
import {
  fetchChatSiteConfig,
  getChatSessionId,
  sendChatMessage,
  ...
  type ChatMessage,
  type ChatSiteConfig,
} from '@/lib/nexrena-chat/chat-api';
import { ChatMessageBody } from '@/lib/nexrena-chat/format-chat-message';
...
export function SiteChatWidget({ siteKey = 'nicoloperena' }: Props) {
  ...
  const [starters, setStarters] = useState<string[]>(siteKey === 'nexrena' ? DEFAULT_STARTERS : []);
  ...
  useEffect(() => {
    ...
    // @ts-expect-error legacy global used across portfolio CTAs
    window.openRecruitingAssistant = openChat;
    return () => {
      // @ts-expect-error legacy global
      delete window.openRecruitingAssistant;
    };
  }, []);
  ...
  const openContactForm = useCallback(() => {
    // @ts-expect-error global contact hub from IntakeModal
          ...
          >
                ...
                <article ref={intakeRef} className="site-chat-msg site-chat-msg-assistant">
                  ...
                  <div className="site-chat-msg-content site-chat-intake-wrap">
                    <ChatIntakeForm
                      ...
                      siteKey={siteKey}
                      teamLabel={config?.label ?? 'Nicholas'}
            ...
            <footer className="site-chat-footer">
              ...
              <p className="site-chat-disclaimer">
                AI can make mistakes. Use the{' '}
                <button type="button" className="site-chat-disclaimer-link" onClick={openContactForm}>
                  contact form
                </button>
