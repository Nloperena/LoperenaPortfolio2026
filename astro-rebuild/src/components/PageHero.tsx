...
export const PageHero = ({ page }: PageHeroProps) => {
  ...
  const minHeight = config.compact
    ...
    : config.simple
      ? 'min-h-[85svh] lg:min-h-[calc(100svh-4rem)]'
    ...
    >
      ...
      >
        <div
          ...
          } ${config.simple ? 'justify-center' : ''}`}
        >
          {config.simple ? (
            <>
              <p className="hero-meta font-mono text-[11px] font-bold uppercase tracking-[0.22em] text-secondary">
                {config.eyebrow}
              </p>

              <h1 className="hero-headline font-mono text-[clamp(2.5rem,8vw,4.25rem)] font-black uppercase leading-[0.95] tracking-tighter text-foreground">
                {config.headlineLines.map((line, index) => (
                  <span key={`${page}-${line}`} className="hero-headline-line block">
                    {line}
                  </span>
                ))}
              </h1>

              <p className="hero-subline max-w-xl font-sans text-base md:text-lg leading-relaxed text-foreground/90">
                {config.subline}
              </p>
